...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=False, stride=8, roll=False, norm=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.roll:
            bgr = np.roll(bgr, (self.stride//4*self.roll, self.stride//4*self.roll), (1, 2))
            hyper = np.roll(hyper, (self.stride//4*self.roll, self.stride//4*self.roll), (1, 2))
...
class ValidDataset(Dataset):
    def __init__(self, data_root, bgr2rgb=False, norm=True):
        self.hypers = []
        self.bgrs = []
        hyper_data_path = f'{data_root}/Train_Spec/'
        bgr_data_path = f'{data_root}/Train_RGB/'
